...
plot(func, col="blue", pch=16, main="Dependency of win rate on rating", xlab="Raing", ylab="Win rate")
...
t.test(x=x, y=y, alternative="two.sided")
```
We get the opposite result. If the one does not do the castling the pronanility of victory becomes smaller.
The p-value is too small, so we can state, that two P parameters are not equal.


#### Dependency on the number of move, castling was made

Let us look at the distribution of the number of move, the castling was made at
...
It looks very similar to some famous distribution, so let us try to find which one.

```{r}
descdist(castling_moves)
```
This graph shows us, from what distribution is our sample likely to be drawn.

It seems to be close to Gamma one, so let us test this

Here, we find the most likely shape and scale parameters for Gamma distribution and plot the graphs for our data and the expected distribution
...
Let us perform the Kolmogorov-Smirnov goodness-of-fit test for determining, how lokely is our data to be drawn from the Gamma distribution
...
```{r}
move_number_success_rate = data.frame(move_number=seq(1, 50), wins=0, number_of_games=0)

for(i in 1:length(df$move_number)){
  if(as.numeric(df$victory[i]) == 3 && df$move_number[i] <= 50){
    move_number_success_rate$wins[df$move_number[i]] = move_number_success_rate$wins[df$move_number[i]] + 1
  }
  if(df$move_number[i] <= 50){
